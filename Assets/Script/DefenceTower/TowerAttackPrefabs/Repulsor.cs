...
public class Repulsor : BaseAttackPrefab
{
    ...
    public override void OnTriggerEnter2D(Collider2D collision)
    {
        if (collision.CompareTag(TagIds.EnemyTag))
        {
            ...
            bool isInAngle = Mathf.Abs(angleDeg - transform.rotation.eulerAngles.z) < tower.BonuseNoise / 2f;

            Debug.Log($"IsinAngle : {isInAngle}");
            Debug.Log($"TargetAngle : {(angleDeg - transform.rotation.eulerAngles.z)}");
            if(isInAngle)
            {
