...
Debug.Log($"경험치보상 전{rewardData.CLEAR_REWARD2_COUNT} / 보너스 {Terraforming.terraformingExpGainBonus} / 최종 {(int)(rewardData.CLEAR_REWARD2_COUNT * Terraforming.terraformingExpGainBonus)}");
        ...
        else
        {
            goldLayout.SetActive(true);
            expLayout.SetActive(true);
            goldText.text = $"+ {(int)(rewardData.CLEAR_REWARD1_COUNT * percent*Terraforming.terraformingGoldGainBonus*0.5f)}";
            expText.text = $"+ {(int)(rewardData.CLEAR_REWARD2_COUNT * percent*Terraforming.terraformingExpGainBonus*0.5f)}";
Debug.Log($"골드보상 전{rewardData.CLEAR_REWARD1_COUNT} / 보너스 {Terraforming.terraformingGoldGainBonus} / 최종 {(int)(rewardData.CLEAR_REWARD1_COUNT * Terraforming.terraformingGoldGainBonus)}");
Debug.Log($"경험치보상 전{rewardData.CLEAR_REWARD2_COUNT} / 보너스 {Terraforming.terraformingExpGainBonus} / 최종 {(int)(rewardData.CLEAR_REWARD2_COUNT * Terraforming.terraformingExpGainBonus)}");
            goldReward = (int)(rewardData.CLEAR_REWARD1_COUNT * percent*Terraforming.terraformingGoldGainBonus*0.5f);
            expReward = (int)(rewardData.CLEAR_REWARD2_COUNT * percent*Terraforming.terraformingExpGainBonus*0.5f);
            diamondReward = 0;
        }
    }
}
